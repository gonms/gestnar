...
class PDF extends FPDF
{
    ...
    function Header()
	{
        ...
        $this->Cell(10,5,'');
        $this->Cell(5,5,'','L');
        $this->Cell(80,5,$this->cliente->getDireccion());
        $this->Cell(5,5,'','R');
        $this->Ln();
    ...
    function Footer()
    {
        ...
        if ($this->PageNo() == $this->num_paginas)
        {
            ...
            if ($this->factura->getFacturaSinIVA() == '0')
            {
                ...
                if ($this->factura->getTipoRetencion() == 'sin_iva')
                {
                    ...
                    $total = number_format($base_iva - $importe_retencion + $importe_iva,1,',','.');
                }
				else //($this->factura->getTipoRetencion() == 'con_iva')
                {
                    ...
                    $total = number_format($base_iva + $importe_iva,1,',','.');                    
                }
                
                $base_iva = number_format($base_iva,1,',','.');
                $importe_iva = number_format($importe_iva,1,',','.');
                $importe_retencion = number_format($importe_retencion,1,',','.');
        ...
        $this->Cell(32,6,'SUBTOTAL','LTR',0,'C');
        $this->Cell(32,6,'DESCUENTO','LTR',0,'C');
        $this->Cell(32,6,'RETENCIÓN','LTR',0,'C');
        $this->Cell(25,6,'BASE I.V.A.','LTR',0,'C');
        $this->Cell(32,6,'I.V.A.','LTR',0,'C');
        $this->Cell(37,6,'TOTAL FACTURA','LTR',0,'C');
        $this->Ln(6);
        $this->Cell(32,5,'','LBR',0,'C');
        $this->Cell(8,5,'%','LBR',0,'C');
        $this->Cell(24,5,'IMPORTE','LBR',0,'C');
        $this->Cell(8,5,'%','LBR',0,'C');
        $this->Cell(24,5,'IMPORTE','LBR',0,'C');
        $this->Cell(25,5,'','LBR',0,'C');
        $this->Cell(8,5,'%','LBR',0,'C');
        $this->Cell(24,5,'IMPORTE','LBR',0,'C');
        $this->Cell(37,5,$moneda,'LBR',0,'C');
        $this->Ln();
        $this->Cell(32,8,$subtotal,'LBR',0,'C');
        $this->Cell(8,8,$porcentaje_descuento,'LBR',0,'C');
        $this->Cell(24,8,$importe_descuento,'LBR',0,'C');
        $this->Cell(8,8,$porcentaje_retencion,'LBR',0,'C');
        $this->Cell(24,8,$importe_retencion,'LBR',0,'C');
        $this->Cell(25,8,$base_iva,'LBR',0,'C');
        $this->Cell(8,8,$porcentaje_iva,'LBR',0,'C');
        $this->Cell(24,8,$importe_iva,'LBR',0,'C');
        $this->Cell(37,8,$total,'LBR',0,'C');        
        $this->Ln();
        $this->Cell(40,6,'FORMA DE PAGO:','',0,'C');
		$num_cuenta = $this->factura->getNumeroCuenta();
        $this->Cell(150,6,$this->cliente->getFormaPago() . " " . $num_cuenta);
        $this->Ln(8);
...
$pdf->num_paginas = ceil((107 + $total_alto) / 252);  //TODO: optimizar
if ($_GET['id_factura'] == "14" || $_GET['id_factura'] == "18") $pdf->num_paginas = 5;
$pdf->AliasNbPages();
$pdf->AddPage();
