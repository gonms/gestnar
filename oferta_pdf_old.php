...
class PDF extends FPDF
{
    ...
    function Header()
	{
	    ...
	    $this->Cell(0,4,'SERVICIOS TÉCNICOS Y COMERCIALES',0,1);    
	    $this->SetFont('Arial','',8);
	    $this->Cell(0,4,'AREA EMPRESARIAL ANDALUCÍA, Sector 1',0,1);
	    ...
	    $this->Ln(5);
	    $this->Cell(7);
	    $this->MultiCell(180,4,'Tenemos el gusto de someter a su consideración nuestra mejor oferta para el posible suministro del material que seguidamente les detallamos:');
	    $this->Ln(5);
	    $this->SetFont('Arial','B',10);
	    $this->Cell(35,5,'OFERTA Nº',1,0,'C');
	...
	function Footer()
	{
	  	if ($this->PageNo() == $this->num_paginas)
        {
			...
			$mercancia_franco = "s/camión en " . $this->oferta->getMercanciaFranco();
	    ...
	    $this->SetFont('Arial','B',8);
	    $this->Cell(45,7,'Mercancía Franco','LTR',0);
		...
		$this->Cell(0,5,'* Conforme a la Ley 15/2010, de 5 de julio, de modificación de la Ley 3/2004, de 29 de diciembre.',0,1,'L');
	    $this->Cell(0,5,'R.M. Madrid, T 7411 General, 6384 de la Secc. 3ª, Lib Soc. F81, Hoja 73668, Inscrip. 1ª, C.I.F. A 78 - 420627',0,1,'C');
...
else if (!empty($_GET['paginas']) && $_GET['paginas'] > 0)
{
...
{
	$pdf->num_paginas = ceil((92 + $total_alto) / 253);
}
...
if ($_GET['id_oferta'] == 328)
	...
	$pdf->num_paginas = 4;
else if ($_GET['id_oferta'] == 707)
	$pdf->num_paginas = 5;
else if ($_GET['id_oferta'] == 747)
	$pdf->num_paginas = 7;
